...
const FlexList = styled.ul`
  ...
  @media (max-width: ${props => props.mobileWidth}) {
    display: flex;
    flex-wrap: nowrap;
    justify-content: ${(props) => { return props.justifyContent || 'flex-start' }};
    align-items: ${(props) => { return props.alignItems || 'stretch' }};
    transition: 500ms transform linear;
    transform: ${(props) => {
    return props.selected !== 0 ? `translateX(${props.selected * -itemPlusPaddingWidthPct}%)` : 'translateX(0)'
  }};
  }
`

...
class Colophon extends React.Component {
  ...
  render() {
    const { relateds, team, topicTitle, topicImage, isFocus } = this.props
    return (
      <Container>
        <Wrapper>
          <RealtedsBox isFocus={isFocus}>
            <TopicBox topicTitle={topicTitle} topicImage={topicImage} />
            <Relateds relateds={relateds} />
          </RealtedsBox>
          <InfosBox isFocus={isFocus}>
            <Team team={team} />
            <Button title="訂閱電子報"><MailIcon /></Button>
            <Button title="分享"><FbIcon /></Button>
            <Button title="開放原始碼"><GithubIcon /></Button>
          </InfosBox>
        </Wrapper>
      </Container>
    )
